...
- 仅存储按日聚合数据，保留最近 30 天；不保存完整请求/响应内容或 API key。
- 不添加运行时第三方依赖。
...
Pi `0.84.2` 的公开扩展 API 不提供原生模型选择器行装饰扩展点或默认模型持久化方法。因此本扩展使用自定义选择器，选择后切换当前 session 模型；不承诺完全复现原生 `/model` 的默认模型保存行为。扩展不集成 `/glance`，也不使用私有 API 或主动测活请求。
...
目前统计存储 schema 版本为 v4，数据目录由 `src/config.ts` 根据 Pi agent 目录确定。